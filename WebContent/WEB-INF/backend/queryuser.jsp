...
<div class="container">
	...
	<form action="queryuser" method="post" class="form-horizontal" role="form">
				...
				<table class="table table-striped">
					...
					<tbody>
						...
						<tr<%=(row.get(4).toString().equals("false"))?" class=\"text-danger\"":"" %>>
							...
							<td><a href="<%=basePath+"rbac/uuser" %>?username=<%=row.get(1)%>"><span
			...
			<div class="text-center">
				<ul class="pagination">
					...
					<%
						if (pageNumber > 1) {
					%>
					<li><a href="queryuser?querytype=<%=request.getAttribute("querytype") %>&queryname=<%=request.getAttribute("queryname") %>&pageNumber=<%=pageNumber - 1%>">&laquo;</a></li>
					<%
						} else {
					%>
					...
					<a href="queryuser?querytype=<%=request.getAttribute("querytype") %>&queryname=<%=request.getAttribute("queryname") %>&pageNumber=<%=i%>"><%=i%></a></li>
					<%
						}
					...
					%>
					<li><a href="queryuser?querytype=<%=request.getAttribute("querytype") %>&queryname=<%=request.getAttribute("queryname") %>&pageNumber=<%=pageNumber + 1%>">&raquo;</a></li>
					<%
						} else {
					%>
...
$(".glyphicon-remove").parent().on("click", function() {
	...
	var del=$(this);
	$.post( "<%=basePath+"rbac/duser" %>", { uname: username}, function( data ) {
         if(data=="ok") {
